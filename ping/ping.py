...
#=============================================================================#
    ...
    #=============================================================================#
    def do_one(self, myStats, destIP, hostname, timeout, mySeqNumber, numDataBytes, quiet = False):
        """
    ...
    def verbose_ping(self, hostname, timeout = 3000, count = 3,numDataBytes = 64, path_finder = False):
        ...
        for i in range(count):
            delay = self.do_one(myStats, destIP, hostname, timeout, mySeqNumber, numDataBytes)
    ...
    def quiet_ping(self,hostname, timeout = 3000, count = 3,
        ...
        if path_finder:
            fakeStats = MyStats()
            self.do_one(fakeStats, destIP, hostname, timeout,
                            mySeqNumber, numDataBytes, quiet=True)
            time.sleep(0.5)
    
        for i in range(count):
            delay = self.do_one(myStats, destIP, hostname, timeout,
                            mySeqNumber, numDataBytes, quiet=True)
        ...
        self.emit('setItem(int, int, int, QString,QString)', myStats.maxTime, myStats.minTime, myStats.avrgTime, myStats.fracLoss )
